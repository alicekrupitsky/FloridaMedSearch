...
public partial class inPatient : System.Web.UI.Page
{
    ...
    {
        // Parse multiple ICD codes safely.
        ...
        var icdList = ParseIcdList(IcdRaw);
        ...
        if (!hasAnyLocationFilter || icdList.Count == 0)
        {
            var sb = new StringBuilder();
            if (!string.IsNullOrEmpty(ZipModeError))
                sb.Append("<p style='color:#b02a37'>").Append(Html(ZipModeError)).Append("</p>");
            sb.Append("<p>Select County and/or Center ZIP + miles, enter ICD-10 procedure code(s), then click Search.</p>");
            return sb.ToString();
        }
...
order by 2 desc";
    ...
    private void WriteIcd10ProcedureDataJson()
    {
...
ORDER BY z.ZipCode;";
    ...
    private static List<string> ParseIcdList(string icdRaw)
    {
        var icdList = new List<string>();
        if (string.IsNullOrEmpty(icdRaw)) return icdList;

        var parts = icdRaw.Split(new char[] { ',', ';', ' ', '\t', '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        foreach (var p in parts)
        {
            ...
            if (seen.Add(code)) icdList.Add(code);
        }
